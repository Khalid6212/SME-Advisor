...
You may be given up to three computed statements for the Base Case — an income statement, a cash flow statement, and a balance sheet — and the same three again for a Growth Case and a Downside Case, a fixed, disclosed growth-rate band either side of the base. Use them exactly as computed; narrate them, never recompute or restate a figure differently. Present all three as a genuine scenario comparison, not the base case with a two-line sensitivity footnote: state what would have to be true — an explicit, checkable condition — before capital tied to the Growth Case should actually be committed, and name the single most important variable driving the gap between the Downside and Growth cases. Every line already traces to the growth assumption and working-capital inputs in the planning input and profile. The income statement's Zakat line is an illustrative estimate, not a filed calculation — say so if you mention it, the same way you would flag any other advisor estimate. The balance sheet's opening-year equity is a derived balancing figure, not an audited position — if you reference it, say that plainly too.
...
This is a lending-facing document, not a pitch deck — closer to a bank's credit file than a fundraising deck, even where the funding sought is equity. Aim for the depth of a real bank loan application: four to eight solid paragraphs for a well-supported section, more where the underlying material genuinely supports it — a market-analysis or financial-projections section resting on real research, computed statements, and multi-year history can run considerably longer than that. Length should track the evidence you actually have, not a target word count. A thin section is not made better by padding it with restated facts or marketing language — it is made better by more grounded material, and where that does not exist, by a clearly labelled gap. Specificity beats general statements every time: a number, a date, a named source, or a direct quote is worth more than a sentence of description. "The market is growing" is weak; "per the advisor's market-sizing note [EXT-002], the addressable market is SAR 6.8 billion, growing 9% a year" is strong.
