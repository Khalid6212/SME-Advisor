...
export const businessPlanTemplate: PlanTemplate = {
  ...
  sections: [
    ...
    },
    {
      ...
      guidance:
        "Market size and growth, using exactly the figures and sources supplied in the market-sizing input — cite the source inline (\"per [source]\") the way the input states it. Never round, extrapolate, or restate a market figure differently than given. If no market-sizing input was supplied, call flag_gap for this section and do not call draft_section for it at all — an invented TAM is exactly the kind of statistic a reader checks first and forgives least, and a hedged paragraph that gestures at the market without a hard number is still working around the gap instead of naming it. Where TAM narrows to SAM narrows to SOM, show the narrowing logic, not three disconnected numbers — geography, segment, and a realistic capture rate, each stated. You have code_execution in this phase: where you need to combine given figures into one you don't already have (a bottom-up estimate from a unit count and a per-unit figure, a penetration-rate calculation), compute it and show the arithmetic in the prose — a number with its working shown is stronger than the same number asserted bare, and it must still trace entirely to figures you were actually given, never an assumed input.",
      draws_on: ["market_position"],
      draftable_from_profile: false,
      required: false,
      audiences: [...BOTH],
    },
    {
      ...
      guidance:
        "Name real competitors and compare on the dimensions that matter to this business — pricing, positioning, scale, geography, whatever actually differentiates them here. Use the advisor's competitor assessment for strengths and weaknesses — the owner's own view of a competitor's weaknesses is not a reliable source and should not be presented as fact. The profile's named competitors and differentiation still ground which competitors matter and how this business positions against them. Where the advisor's research gives you figures to compare on a concrete dimension (price, capacity, geographic reach), use code_execution to work out the actual comparison — a percentage difference, a ratio — rather than describing each competitor in turn and leaving the reader to do the arithmetic.",
